...
class UnidadDireccionController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('unidaddireccion.index', compact('lista','searchtype','search','sort','ruta'));
    }

    public function create()
    {
        $array = [0,1];
        $tipoUnidad = $this->tipoUnidadService->getTipoUnidadByTipo(2);
        ...
        $unidadDireccion = new Unidad();
        $unidadDireccion->und_id = 0;
        $unidadDireccion->estado = 'AC';
        $unidadDireccion->latitud = $param->valor2;
        $unidadDireccion->longitud = $param->valor3;

        return view('unidaddireccion.createedit',compact('unidadDireccion','listaBiografias','listaComboUnidades','tipoUnidad'));
    }
    ...
    public function edit($und_id)
    {
        ...
        $listaComboUnidades = $this->unidadService->getAllAcUnidadesParaComboSelectOrdenadosByArrayTipoUnidad($array);
        $listaBiografias=$this->biografiaService->getComboBiografia()->prepend('Sin Biografía','');
        $unidadDireccion = $this->unidadService->getById($und_id);
        $imagenesBanners = $this->imagenUnidadService->getImagenUnidadBannerByUnidadAndTipo($unidadDireccion->und_id,$tipo_imagen_banner);
        $cantidadimageneshay = count($imagenesBanners);
        return view('unidaddireccion.createedit',compact('unidadDireccion','listaComboUnidades','listaBiografias','imagenesBanners','cantidadimageneshay'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->und_id == 0 ) {
            ...
            $validator = Validator::make($data, [
                ...
                'organigrama'=> 'mimes:jpeg,jpg,JPG,JPEG|max:4000'
            ], $messages);
            ...
            try {
                ...
                if (empty($unidad)){
                    ...
                    return redirect('sisadmin/unidaddireccion');
                }
            }catch (Exception $e){
            ...
            if ($request->hasFile('organigrama')) {
                ...
                $validator = Validator::make($data, ['organigrama' => 'mimes:jpeg,jpg,JPG,JPEG|max:4000' ], $messages);
                if ($validator->fails()){
            ...
            try {
                ...
                if (empty($unidad)){
                    Toastr::warning('No se pudo editar la unidad Direccion', "");
                    return back()->withInput();
                }else{
                    Toastr::success('Operación completada ', "");
                    return redirect('sisadmin/unidaddireccion/edit/'.$request->und_id);
                }
            }catch (Exception $e){
                Log::error($e->getMessage());
                Toastr::error('Ocurrio un error al editar la unidad Direccion', "");
                return back()->withInput();
            }
        }
    }
    ...
    public function _eliminarimagen_unidad(Request $request)
    {
        ...
        $cantidadimageneshay = count($imagenesBanners);
        return view('unidaddireccion._tablaimagenunidad',compact('imagenesBanners','cantidadimageneshay'));
    }
    ...
    public function editar($und_id)
    {
        ...
        $imagenesBanners = $this->imagenUnidadService->getImagenUnidadBannerByUnidadAndTipo($unidadDireccion->und_id,$tipo_imagen_banner);
        $cantidadimageneshay = count($imagenesBanners);
        return view('unidaddireccion.createeditar',compact('unidadDireccion','listaComboUnidades','listaBiografias','imagenesBanners','cantidadimageneshay','unidad'));
    }
        ...
        if($request->und_id == 0 ) {
            ...
            try {
                ...
                if (empty($unidad)){
                    ...
                    Toastr::success('Operación completada ', "");
                    return redirect('sisadmin/unidaddireccion/editar/'.$request->und_id);
                }
            }catch (Exception $e){
                Log::error($e->getMessage());
                Toastr::error('Ocurrio un error al editar la unidad Direccion', "");
