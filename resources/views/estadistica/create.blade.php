...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-8">
            @if ($estadistica->est_id == 0)
                <h3 align="center">Nueva Estadística</h3>
            @else
                <h3 align="center">Editar Estadística</h3>
            @endif
            <br>
            <form class="form-horizontal" id="formulario" autocomplete="off" method="POST" enctype="multipart/form-data" action="{{ url('sisadmin/estadistica/store') }}">
                {{ csrf_field() }}
                <div class="form-group row">
                    <label class="col-md-1 col-form-label"></label>
                    <label class="col-md-2 col-form-label">Título*:</label>
                    <div class="col-md-8">
                        {{Form::hidden('est_id',$estadistica->est_id)}}
                        {{Form::hidden('und_id',$estadistica->und_id)}}
                        {{Form::hidden('estado',$estadistica->estado)}}
                        {{Form::hidden('publicar',$estadistica->publicar)}}
                        <input type="text" value="{{ old('titulo',$estadistica->titulo) }}" class="form-control form-control-sm"  name="titulo" id="titulo" required >
                        @error('titulo')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <input type="text" value="{{ old('autor',$estadistica->autor) }}" class="form-control form-control-sm"  name="autor" id="autor" required >
                        @error('autor')
                        <p class="form-text text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <textarea class="form-control form-control-sm" rows="3" cols="40" name="descripcion" id="descripcion">{{ old('descripcion',$estadistica->descripcion) }}</textarea>
                        @error('descripcion')
                        <p class="form-text text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                @else
                    <div class="form-group row">
                        ...
                        <div class="col-md-8">
                            {{
                                Html::image(asset('storage/uploads/'.$estadistica->imagen), 'Sin Imagen', array('id'=>'imagen', 'class' =>'img-thumbnail','width'=>'200'))
                            }}
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-8">
                            <a target="_blank" href="{{asset('storage/uploads/'.$estadistica->archivo)}}"><i class="fa fa-download"></i> Descargar Archivo</a>
                        </div>
                    </div>
                    <br>
                    <div class="form-group row">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <input onkeyup="return false;" type="text" value="{{ old('fecha',date('d/m/Y',strtotime($estadistica->fecha))) }}" class="form-control form-control-sm"  name="fecha" id="fecha" required >
                        @error('fecha')
                        <p class="form-text text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="row justify-content-center" style="margin-top: 10px;">
                    ...
                    <div class="col-md-2">
                        <a href="{{ url('sisadmin/estadistica/'.$unidad->und_id.'/lista')}}" class="btn btn-danger">Cancelar</a>
                    </div>
                </div>
            </form>
        </div>
