...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Mi Unidad / Publicaciones</li>
        </ol>
    </nav>
    <!-- Breadcrumb Area End-->
    <h3 align="center">Publicaciones Cientificas: {{$unidad->nombre}}</h3>
    <div class="col-md-12">
        ...
        <div class="col-md-12">
            <div class="row">
                @if (verificarAcceso(44))
                    <a href="{{ url('sisadmin/publicacioncientifica/create/'.$unidad->und_id) }}" class="btn btn-primary btn-sm" id="btnNuevo"><i class="fa fa-plus"></i> Agregar</a>
                @else
                ...
                <table class="table table-hover table-responsive-xl table-sm" id="tablaContenido">
                    ...
                    @foreach ($lista as $key => $item)
                        <tr>
                            ...
                            <td>
                                @if (verificarAcceso(46))
                                    {{
                                        Form::select('publicar',$publicar,$item->publicar,['class'=>'form-control form-control-sm','id'=>'publicar','onchange'=>"cambiarPublicar('$item->puc_id',this.value);"])
                                    }}
                                @else
                            ...
                            <td>
                                @if (verificarAcceso(45))
                                    <a href="{{ url('sisadmin/publicacioncientifica/edit/'.$item->puc_id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> Editar</a>
                                @else
                                    <a href="#" class="btn btn-primary btn-sm disabled"><i class="fa fa-edit"></i> Editar</a>
                                @endif
                                @if (verificarAcceso(47))
                                    <button type="button" class="btn btn-danger btn-sm" onclick="cambiarEstado('{{$item->puc_id}}','EL')"><i class="fa fa-trash"></i> Eliminar</button>
                                @else
    ...
    <script type="text/javascript">
        ...
        function cambiarEstado(id,estado){
            $.confirm({
                theme: 'modern',
                title: false,
                content: '¿Desea eliminar esta publicación cientifica?',
                buttons: {
                    SI: {
                        text: 'SI',
                        btnClass: 'btn-blue',
                        ...
                        action: function(){
                            ...
                            $.ajax({
                                url : '{{url("sisadmin/publicacioncientifica/_cambiarEstado")}}',
                                data : {
                                    id:id,
                                    estado:estado
                                },
        ...
        function cambiarPublicar(puc_id,publicar){
            console.log(puc_id+' '+publicar);
            loaderR.showPleaseWait();
            $.ajax({
                url : '{{url("sisadmin/publicacioncientifica/_cambiarPublicar")}}',
                data : {
                    puc_id:puc_id,
                    publicar:publicar
                },
