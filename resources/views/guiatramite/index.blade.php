...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Mi Unidad / Guía Tramites</li>
        </ol>
    </nav>
    <!-- Breadcrumb Area End-->
    <h3 align="center">Guías Trámites: {{$unidad->nombre}}</h3>
    <div class="col-md-12">
        ...
        <div class="col-md-12">
            <div class="row">
                @if (verificarAcceso(49))
                    <a href="{{ url('sisadmin/guiatramite/create/'.$unidad->und_id) }}" class="btn btn-primary btn-sm" id="btnNuevo"><i class="fa fa-plus"></i> Agregar</a>
                @else
                ...
                <table class="table table-hover table-responsive-xl table-sm" id="tablaContenido">
                    ...
                    @foreach ($lista as $key => $item)
                        <tr>
                            ...
                            <td>
                                @if (verificarAcceso(51))
                                    {{
                                        Form::select('publicar',$publicar,$item->publicar,['class'=>'form-control form-control-sm','id'=>'publicar','onchange'=>"cambiarPublicar('$item->gut_id',this.value);"])
                                    }}
                                @else
                            ...
                            <td>
                                @if (verificarAcceso(50))
                                    <a href="{{ url('sisadmin/guiatramite/edit/'.$item->gut_id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> Editar</a>
                                @else
                                    <a href="#" class="btn btn-primary btn-sm disabled"><i class="fa fa-edit"></i> Editar</a>
                                @endif
                                @if (verificarAcceso(52))
                                    <button type="button" class="btn btn-danger btn-sm" onclick="cambiarEstado('{{$item->gut_id}}','EL')"><i class="fa fa-trash"></i> Eliminar</button>
                                @else
    ...
    <script type="text/javascript">
        ...
        function cambiarEstado(id,estado){
            $.confirm({
                theme: 'modern',
                title: false,
                content: '¿Desea eliminar esta Guía de Trámite?',
                buttons: {
                    SI: {
                        text: 'SI',
                        btnClass: 'btn-blue',
                        ...
                        action: function(){
                            ...
                            $.ajax({
                                url : '{{url("sisadmin/guiatramite/_cambiarEstado")}}',
                                data : {
                                    id:id,
                                    estado:estado
                                },
        ...
        function cambiarPublicar(gut_id,publicar){
            console.log(gut_id+' '+publicar);
            loaderR.showPleaseWait();
            $.ajax({
                url : '{{url("sisadmin/guiatramite/_cambiarPublicar")}}',
                data : {
                    gut_id:gut_id,
                    publicar:publicar
                },
