                    ...
                    <div class="table-responsive">
                        <table id="semanario" class="table table-striped table-hover" style="width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $gobernaciontv->url_radio }}</td> <!-- Cambiado a gobernaciontv -->
                                    <td>{{ $gobernaciontv->descripcion }}</td> <!-- Cambiado a gobernaciontv -->
                                    <td>{{ ucfirst($gobernaciontv->estado) }}</td> <!-- Cambiado a gobernaciontv -->
                                    <td>
                                        <a href="{{ route('radiotv.edit', $gobernaciontv) }}" class="btn btn-sm btn-primary">Editar</a>
                                        <!-- Botón para eliminar -->
                                        <form action="{{ route('radiotv.destroy', $gobernaciontv) }}" method="POST" style="display: inline;"> <!-- Cambiado a gobernaciontv -->
                                            @csrf
                                            @method('DELETE')
